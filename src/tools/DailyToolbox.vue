    ...
    <main class="dt-body">
      ...
      <section v-show="activeTab === 'calc'" class="dt-panel calc-panel">
        ...
        <div class="calc-keyboard" :class="{ scientific: calcScientific }">
          ...
          <div class="calc-std-keys">
            <button class="calc-key mem" @click="calcMem('mc')">{{ $t('calc.mc') }}</button>
            <button class="calc-key mem" @click="calcMem('m+')">{{ $t('calc.mPlus') }}</button>
            <button class="calc-key mem" @click="calcMem('m-')">{{ $t('calc.mMinus') }}</button>
            <button class="calc-key mem" @click="calcMem('mr')">{{ $t('calc.mr') }}</button>

            <button class="calc-key red" @click="calcClear">{{ $t('calc.ac') }}</button>
            ...
            <button class="calc-key red" @click="calcOp('±')">{{ $t('calc.plusMinus') }}</button>
            <button class="calc-key op" @click="calcOp('÷')">{{ $t('calc.divide') }}</button>
            ...
            <button class="calc-key op" @click="calcOp('×')">{{ $t('calc.multiply') }}</button>
            ...
            <button class="calc-key op" @click="calcOp('−')">{{ $t('calc.minus') }}</button>
            ...
            <button class="calc-key op" @click="calcOp('+')">{{ $t('calc.plus') }}</button>

            <button class="calc-key" @click="calcNum('%')">{{ $t('calc.percent') }}</button>
            <button class="calc-key" @click="calcNum('0')">0</button>
            <button class="calc-key" @click="calcDot">{{ $t('calc.dot') }}</button>
            <button class="calc-key eq" @click="calcEqual">{{ $t('calc.equal') }}</button>
          </div>
        </div>
      </section>
      ...
      <section v-show="activeTab === 'rate'" class="dt-panel rate-panel">
        <div class="rate-card">
          ...
          <div class="rate-footer">
            {{ $t('rate.dataSource', { time: rateUpdated }) }}
          </div>
        </div>
      ...
      <section v-show="activeTab === 'astro'" class="dt-panel astro-panel">
        ...
        <div class="dt-card lunar-card">
          <div class="lunar-row">
            <div class="label">{{ $t('astro.lunarDate') }}</div>
            <div class="value">{{ lunarInfo.date }}</div>
          </div>
          <div class="lunar-row">
            <div class="label">{{ $t('astro.zodiacGanZhi') }}</div>
        ...
        <div class="dt-card">
          ...
          <div class="base-row">
            <select v-model="baseFrom">
              <option value="10">{{ $t('base.dec') }}</option>
              <option value="2">{{ $t('base.bin') }}</option>
              <option value="8">{{ $t('base.oct') }}</option>
              <option value="16">{{ $t('base.hex') }}</option>
            </select>
            <input type="text" v-model="baseInput" :placeholder="$t('base.inputPlaceholder')" />
          </div>
          <div class="base-result">
            <div>{{ $t('base.bin') }}：{{ baseResults.bin }}</div>
            <div>{{ $t('base.oct') }}：{{ baseResults.oct }}</div>
            <div>{{ $t('base.dec') }}：{{ baseResults.dec }}</div>
            <div>{{ $t('base.hex') }}：{{ baseResults.hex }}</div>
          </div>
        </div>
      </section>
      ...
      <section v-show="activeTab === 'color'" class="dt-panel color-panel">
        ...
        <div class="dt-card">
          <h3>🎨 {{ $t('color.title') }}</h3>
          <div class="color-row">
            <label>{{ $t('color.hex') }}</label>
            <input v-model="colorHex" @input="syncColor('hex')" :placeholder="$t('color.hexPlaceholder')" />
          </div>
          <div class="color-row">
            <label>{{ $t('color.rgb') }}</label>
            <input v-model="colorRgb" @input="syncColor('rgb')" :placeholder="$t('color.rgbPlaceholder')" />
          </div>
          <div class="color-row">
            <label>{{ $t('color.hsl') }}</label>
            <input v-model="colorHsl" @input="syncColor('hsl')" :placeholder="$t('color.hslPlaceholder')" />
          </div>
        </div>
      </section>
    </main>
...
/* ===== Tabs ===== */
const tabs = [
  { key: 'calc', label: t('calc.tab') },
  { key: 'rate', label: t('rate.tab') },
  { key: 'astro', label: t('astro.tab') },
  { key: 'base', label: t('base.tab') },
  { key: 'color', label: t('color.tab') },
]
