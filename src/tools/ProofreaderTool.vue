...
async function handleFile(file) {
  ...
  if (ext === '.docx') {
    try {
      ...
      content.value = result.value.replace(/\r\n/g, '\n')
    } catch {
      alert('解析 .docx 需要 mammoth 库，请运行：npm install mammoth')
      return
    }
  } else {
    content.value = (await file.text()).replace(/\r\n/g, '\n')
  }
...
function runCheck() {
  ...
  const add = (id, typeName, message, pos, len, line, severity) => {
    const ctx = text.slice(Math.max(0, pos - 15), Math.min(text.length, pos + len + 15))
    issues.value.push({ id, typeName, message, pos, len, line, severity, context: ctx })
  }

  // Helper function to safely execute regex match
  const execRegex = (re, callback) => {
    re.lastIndex = 0 // 防游标卡死重用
    let m
    while ((m = re.exec(text)) !== null) {
      callback(m)
      if (m.index === re.lastIndex) re.lastIndex++ // 防死循环
    }
  }

  // 1. Mixed punctuation
  if (enabled.has('mixedPunc')) {
    execRegex(/[\u4e00-\u9fa5][,.!?;:][\u4e00-\u9fa5]|[\u4e00-\u9fa5][\u0020]*[,.!?;:]/g, m => {
  ...
  // 3. Extra spaces (行首/行尾空格)
  if (enabled.has('extraSpace')) {
    execRegex(/^[ \t]+(?!\s*$)/gm, m => {
      add('extraSpace', '多余空格', '行首有空格', m.index, m[0].length, lineOf(m.index), 'warn')
    })
    execRegex(/[ \t]+$/gm, m => {
      add('extraSpace', '多余空格', '行尾有空格', m.index, m[0].length, lineOf(m.index), 'warn')
    })
  }

  // 4. Consecutive spaces (文本中间连续空格)
  if (enabled.has('consecSpace')) {
    execRegex(/[ \t]{2,}/g, m => {
      const isStart = m.index === 0 || text[m.index - 1] === '\n'
      const isEnd = m.index + m[0].length === text.length || text[m.index + m[0].length] === '\n'
      if (!isStart && !isEnd) {
  ...
  // 5. Consecutive blank lines (超过两个连续换行，即连续2个及以上空行)
  if (enabled.has('blankLines')) {
    execRegex(/(\n\s*){3,}/g, m => {
      add('blankLines', '连续空行', '超过两个连续空行', m.index, m[0].length, lineOf(m.index), 'suggest')
    })
  }

  // 6. Missing space between Chinese and English
  if (enabled.has('cjSpace')) {
    execRegex(/[\u4e00-\u9fa5][a-zA-Z0-9]|[a-zA-Z0-9][\u4e00-\u9fa5]/g, m => {
...
function fixOne(issue) {
  ...
  switch (issue.id) {
    ...
    case 'consecPunc':
      ...
      break
    case 'extraSpace':
      replacement = '' // 清除行首行尾空格
      break
    case 'consecSpace':
      replacement = ' ' // 多个空格压成单空格
      break
    case 'blankLines':
      replacement = '\n\n' // 压成最多保留1个空行（两个换行符）
      break
    ...
    case 'fullHalf':
      // 半角替换成全角标点
      const puncMap = { ',': '，', '.': '。', '!': '！', '?': '？', ';': '；', ':': '：', '(': '（', ')': '）', '[': '【', ']': '】' }
      replacement = puncMap[target] || target
      break
    case 'dupWord':
...
function fixAll() {
  const fixableIds = ['mixedPunc', 'consecPunc', 'extraSpace', 'consecSpace', 'blankLines', 'cjSpace', 'cnNumSpace', 'fullHalf', 'dupWord']

  // 反向排序（从后往前替换），防止坐标偏移
  const sorted = [...issues.value]
    .filter(i => fixableIds.includes(i.id))
    .sort((a, b) => b.pos - a.pos)

  if (!sorted.length) return

  // 批量一次性原子替换
  let updatedText = content.value
  sorted.forEach(issue => {
    let replacement = ''
    const target = updatedText.slice(issue.pos, issue.pos + issue.len)

    switch (issue.id) {
      case 'mixedPunc':
        replacement = target.replace(/,/g, '，').replace(/\./g, '。').replace(/!/g, '！').replace(/\?/g, '？').replace(/;/g, '；').replace(/:/g, '：')
        break
      case 'consecPunc': replacement = target[0]; break
      case 'extraSpace': replacement = ''; break
      case 'consecSpace': replacement = ' '; break
      case 'blankLines': replacement = '\n\n'; break
      case 'cjSpace':
      case 'cnNumSpace':
        replacement = target[0] + ' ' + target[1]
        break
      case 'fullHalf':
        ...
        replacement = puncMap[target] || target
        break
      case 'dupWord': replacement = target.slice(0, target.length / 2); break
    }
    updatedText = updatedText.slice(0, issue.pos) + replacement + updatedText.slice(issue.pos + issue.len)
  })
...
function exportTxt() {
  ...
  a.href = url
  a.download = (fileName.value ? fileName.value.replace(/\.[^.]+$/, '') : 'proofread') + '_fixed.txt'
  a.click()
