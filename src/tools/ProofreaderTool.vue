...
function fixOne(issue, skipCheck = false) {
  const text = content.value
  let replacement = ''
  ...
  if (!skipCheck) runCheck()
}

function fixAll() {
  // Sort by position descending to avoid index shift
  const sorted = [...issues.value]
    .filter(i => ['mixedPunc', 'consecPunc', 'extraSpace', 'consecSpace', 'cjSpace', 'cnNumSpace', 'dupWord'].includes(i.id))
    .sort((a, b) => b.pos - a.pos)

  sorted.forEach(issue => fixOne(issue, true))
  runCheck()
}
