...
function runCheck() {
  ...
  // 3. Extra spaces (line start/end) - 使用精确正则检索
  if (enabled.has('extraSpace')) {
    ...
    const tailRe = /[ \t]+$/gm
    let m2
    while ((m2 = tailRe.exec(text)) !== null) {
      add('extraSpace', '多余空格', '行尾有空格', m2.index, m2[0].length, lineOf(m2.index), 'warn')
    }
  }

  // 4. Consecutive spaces (中间连续空格)
  ...
  // 5. Consecutive blank lines (超过两个连续换行，即连续3个及以上 \n)
  if (enabled.has('blankLines')) {
    const re = /(\n\s*){3,}/g
    let m
    while ((m = re.exec(text)) !== null) {
      add('blankLines', '连续空行', '存在多余连续空行', m.index, m[0].length, lineOf(m.index), 'suggest')
    }
  }
...
function fixAll() {
  // 按索引从大到小排序倒序替换，防坐标偏移
  const fixableIds = ['mixedPunc', 'consecPunc', 'extraSpace', 'consecSpace', 'blankLines', 'cjSpace', 'cnNumSpace', 'dupWord']
