        ...
        <div class="ref-list">
          <div v-for="(ref, idx) in references" :key="ref.id" class="ref-item">
            ...
            <div class="ref-fields">
              <input v-model="ref.authors" :placeholder="$t('citation.authors')" />
              <input v-model="ref.year" :placeholder="$t('citation.year')" type="number" />
              <input v-model="ref.title" :placeholder="$t('citation.title')" />
              <input v-model="ref.source" :placeholder="$t('citation.source')" />
              <input v-model="ref.volume" :placeholder="$t('citation.volume')" />
              <input v-model="ref.doi" :placeholder="$t('citation.doi')" />
              <select v-model="ref.type">
                <option value="journal">{{ $t('citation.journal') }}</option>
                <option value="book">{{ $t('citation.book') }}</option>
                <option value="conference">{{ $t('citation.conference') }}</option>
                <option value="web">{{ $t('citation.web') }}</option>
                <option value="thesis">{{ $t('citation.thesis') }}</option>
              </select>
              <textarea
...
function autoCite(refIdx) {
  ...
  if (idx >= 0) {
    ...
    const mark = generateCiteMark(refIdx)
    rawText.value = before + mark + after
