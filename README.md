...
| 功能 | 说明 |
|------|------|
| **🧮 计算器** | 标准模式 + 科学模式（仅 iPad / Mac / 电脑端显示），支持汇率换算、生日星座查询、进制转换（二/八/十/十六进制互转）、颜色编码转换（HEX / RGB / HSL 互转） |
| **⏱️ 倒计时** | 正计时 / 倒计时 |
| **🎲 随机数** | 指定范围随机数生成 |
...
| 功能 | 说明 |
|------|------|
| **📄 代码格式化** | 上传大文件后按指定字节数拆分超长行，避免浏览器渲染崩溃。支持自定义每行字节数，实时预览与下载 |
| **🔗 JS 网址转换** | 输入 JS 文件 URL 获取内容，支持下载为 .js 或 .txt 格式 |
...
| 代码格式化 | `.js` `.ts` `.html` `.css` `.vue` `.py` `.java` `.json` `.xml` `.md` 等 | 格式化后文本 | 按字节拆分超长行，防止浏览器崩溃 |
| JS 网址转换 | JS 文件 URL | `.js` / `.txt` | 获取远程 JS 内容并下载 |
...
> [!WARNING]
> **DATA_VERSION 升级机制**
> `src/stores/app.js` 中的 `DATA_VERSION` 每次数据结构变更（如新增持久化字段）时必须 +1，否则老用户本地缓存不会自动刷新，可能导致新功能（如引用助手、代码工具）无法显示。
...
> [!TIP]
> **代码格式化大文件处理**
> 上传 minified 文件（如 `isomorphic-git.min.js`）时，单行可能长达数万个字节，浏览器渲染会直接崩溃。使用「代码格式化」工具，设置每行 500~2000 字节后点击「格式化拆分」，即可安全查看和下载。

> [!NOTE]
> **JS 网址转换跨域限制**
> 部分 CDN 或 GitHub Raw 链接存在 CORS 限制，无法直接 fetch 获取。此时工具会提示错误，建议手动下载后使用「代码格式化」查看。
